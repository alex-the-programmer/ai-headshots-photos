...
const CustomPicker = ({
  ...
  const renderItem = ({ item, index }: { item: PickerProp, index: number }) => {
    ...
    if (category === "color") {
      ...
      // Debug logging
      console.log('Color Picker Item:', {
        colorName: item.name,
        colorValue,
        relatedValue,
        relatedOutfitName,
        outfitIconName,
        hasRelatedOutfit: !!relatedOutfitOption
      });
            ...
            <>
              ...
              {relatedValue && (() => {
                ...
                console.log('Color Picker Button:', {
                  selectedColor: selectedOption?.name,
                  relatedValue,
                  relatedOutfitName: options.find(opt => opt.value === relatedValue)?.name,
                  outfitIconName: getIconForOption(options.find(opt => opt.value === relatedValue)?.name || "")
                });
